-- ============================================================
-- 01_feature_engineering.sql (Optimized & Updated)
-- ============================================================
...
-- ============================================================
-- 3. USER FEATURES (OPTIMIZED WITH TEMP TABLE FOR LOW RAM)
-- ============================================================

-- ساخت جدول موقت برای جلوگیری از اسکن‌های تکراری جدول حجیم لاگ‌ها
...
-- ایندکس روی جدول موقت برای سرعت بالا در محاسبات بعدی
CREATE INDEX idx_temp_user_base_user ON temp_user_base(user_id);


-- جابجایی دقیق CREATE TABLE به اینجا (قبل از کلمه WITH)
...
-- سلکت نهایی که ویژگی‌ها را کنار هم می‌چیند
SELECT
    e.user_id,
    e.total_events,
    e.total_sessions,
...
-- پاکسازی جدول موقت پس از اتمام کار
DROP TABLE temp_user_base;
...
ON analytics.feature_user_product(user_id, product_id);
...
ON analytics.feature_user_category(user_id, category_id);
...
aspect_stats AS (
    SELECT
        ...
        COUNT(ca.aspect_id) FILTER (WHERE LOWER(ca.sentiment) = 'positive') AS positive_aspect_mentions,
        COUNT(ca.aspect_id) FILTER (WHERE LOWER(ca.sentiment) = 'negative') AS negative_aspect_mentions,
        COUNT(ca.aspect_id) FILTER (WHERE LOWER(ca.sentiment) = 'neutral') AS neutral_aspect_mentions,
...
SELECT
    ...
    COUNT(*) FILTER (WHERE LOWER(ca.sentiment) = 'positive') AS positive_mentions,
    COUNT(*) FILTER (WHERE LOWER(ca.sentiment) = 'negative') AS negative_mentions,
    COUNT(*) FILTER (WHERE LOWER(ca.sentiment) = 'neutral') AS neutral_mentions,
    AVG(ca.negative_pct) AS avg_negative_pct,
    AVG(ca.neutral_pct) AS avg_neutral_pct,
    AVG(ca.positive_pct) AS avg_positive_pct
FROM public.comments c
...
ON analytics.feature_product_aspect(product_id, term);
...
FROM public.products p
LEFT JOIN public.comments c ON p.id = c.product_id
LEFT JOIN public.comment_aspects ca ON c.id = ca.comment_id
GROUP BY p.brand_id;
...
ON analytics.feature_time(hour, iso_weekday);
