...
def run(
    ...
    if not messages or messages[0].get("role") != "system":
        # get_reference_date() از cache می‌خونه (بعد از اولین بار که یک
        # پردازه واقعاً محاسبه‌اش کرده) -- یعنی این خط هزینه‌ی DB روی هر
        # مکالمه‌ی جدید نمی‌ذاره، فقط بار اولِ کل پردازه. تاریخ فقط یک‌بار
        # در همین‌جا (شروع مکالمه) در پرامپت قفل می‌شه، نه هر پیام.
        reference_date = get_reference_date()
        system_prompt = (
            f"{AGENT_SYSTEM_PROMPT}\n\n"
            f"توجه مهم: این دیتاست real-time نیست. \"امروز\" برای همه‌ی "
            f"محاسبات زمانی نسبی (مثل \"۳۰ روز اخیر\") باید {reference_date.isoformat()} "
            f"در نظر گرفته بشه -- نه تاریخ واقعی امروز. هرگز از "
            f"NOW()/CURRENT_DATE واقعی Postgres در SQL استفاده نکن."
        )
...
def main() -> None:
    ...
    result = run("چرا فروش دفترچه یادداشت ها در ماه جاری کاهش پیدا کرده؟", chat_id="demo-chat-1")
    print("\nFINAL ANSWER:")
