...
class ProductionSQLValidator:
    def __init__(self, dialect: str = "postgres"):
        ...
        # ۱. نگاشت کامل روابط یک-به-چند (Parent -> Child)
        ...
        # 👈 [اضافه شده] نگاشت کلیدهای مجاز برای شرط JOIN
        ...
        # ۳. ستون‌های درصدی و نسبتی پیش‌محاسبه‌شده
        ...
        # ۴. اسکیما کامل بر اساس تمامی DDLها
        self.schema = {
            "public": {
                "cities": ["city_id", "name"],
                "users": ["user_id"],
    ...
    def _is_safe_division(self, right_expr: exp.Expression) -> bool:
        return bool(right_expr.find(exp.Nullif) or right_expr.find(exp.Case))

    def _resolve_column_table(self, col: exp.Column, tables_in_scope: Dict[str, str]) -> str:
        col_table = col.table.lower() if col.table else ""
        col_db = col.db.lower() if col.db else ""
    ...
    def validate(self, sql_query: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            # جلوگیری از اجرای چند دستور در یک کوئری (ممانعت از SQL Injection و Multi-Statement)
            if len(parsed_statements) > 1:
                return False, ["SECURITY ERROR: Multi-statement SQL queries are strictly forbidden."]

            parsed = parsed_statements[0]
            
            # 🛑 لایه امنیتی اصلی: مسدودسازی تمامی دستورات غیر SELECT (از جمله DELETE, DROP, INSERT, UPDATE, ALTER)
        ...
        # ۳. پیمایش ساختار یافته اسکوپ‌ها با استفاده از traverse_scope
        for scope in traverse_scope(qualified_parsed):
            ...
            # 👈 [اضافه شده] بررسی صحّت شرط‌های ON در JOIN
                    ...
                    for eq in on_clause.find_all(exp.EQ):
                        ...
                        if pair in self.allowed_join_keys:
                            ...
                            if (left_col, right_col) not in valid_keys:
                                errors.append(f"INVALID JOIN CONDITION: Cannot join '{left_table}' and '{right_table}' on '{left_col} = {right_col}'.")
                            ...
                            if (right_col, left_col) not in valid_keys:
                                errors.append(f"INVALID JOIN CONDITION: Cannot join '{right_table}' and '{left_table}' on '{right_col} = {left_col}'.")

            # بررسی Fan-out در توابع تجمیعی SUM, AVG, COUNT
            ...
            # بررسی Sibling Fan-out (جوین همزمان چند جدول فرزند به یک والد)
            children_for_parent = {}
            for parent, child in self.one_to_many_map:
                if parent in present_tables and child in present_tables:
