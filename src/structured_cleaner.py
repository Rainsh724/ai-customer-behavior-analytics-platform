...
class EcomDataCleaner:
    ...
    def _standardize_dates(self, series: pd.Series) -> pd.Series:
        ...
        def convert_to_gregorian(val):
            ...
            # اگر تاریخ از قبل میلادی است (مثل لاگ رفتار کاربران)
    ...
    def clean_structured_data(self, df: pd.DataFrame, dataset_name: str) -> pd.DataFrame:
        ...
        # دریافت شناسنامه جدول از فیچر مپ
        ...
        # استانداردسازی اولیه تمام رشته‌های متنی جدول
        ...
        if config:
            ...
            # تبدیل شناسه‌ها به Int64 برای هماهنگی با SQL
            id_cols = []
            if pk and pk in df.columns:
                id_cols.append(pk)
