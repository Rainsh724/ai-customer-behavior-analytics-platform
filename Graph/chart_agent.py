## PATH: app/graph/chart_agent.py
"""
پیاده‌سازی واقعی Tool_Chart.

نکته درباره‌ی نام‌گذاری: این فایل قبلاً bi_agent.py بود و لینک Power BI
می‌ساخت. الان دیگه هیچ ارتباطی با Power BI/BI نداره -- مستقیم با کد
پایتون از روی نتیجه‌ی یک کوئری SQL نمودار می‌سازه. به همین خاطر تغییر
نام داده شد (bi_agent -> chart_agent، run_bi_tool -> run_chart_tool،
tool_bi -> tool_chart در tools.py) تا اسم با کارکرد واقعی هم‌خونی داشته
باشه و کسی فکر نکنه جایی BI واقعی (Power BI/Tableau/...) درگیره.
...
echarts_option / plotly_figure) به‌علاوه‌ی raw_data (ردیف‌های خام SQL،
برای هر پردازش دیگه‌ای که فرانت‌اند/دولوپر بعداً لازم داشته باشه) تا
فرانت‌اند هرکدوم از این سه کتابخانه رو که استفاده می‌کنه، مستقیم بتونه
بدون تبدیل اضافه رندرش کنه.

مثل tool_sql، اینجا هم خودِ Agent مستقیم SQL می‌نویسه (نه یک زیرایجنت
پنهان) -- description ابزار tool_chart فقط یک ارجاع کوتاه به قوانین/
...
def run_chart_tool(
    ...
    plotly_figure) به‌علاوه‌ی raw_data (ردیف‌های خام) -- مستقیم به‌صورت
    JSON در پیام "tool" به Agent برمی‌گرده و از همون‌جا به کاربر/فرانت‌اند
    می‌رسه.
    """
    if chart_type not in VALID_CHART_TYPES:
        logger.warning("run_chart_tool: chart_type نامعتبر '%s' -- fallback به 'bar'", chart_type)
        chart_type = "bar"
